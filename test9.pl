...
#print "\n$worksheetno\n";
    my $workbook  = Spreadsheet::WriteExcel->new('AlextNet_Comparison_perrow_9_5.xls');
...
#$worksheet->write( "K55", "NV" ,  $format); 
		   ...
		   if(($flag==1) &&(($token cmp "[INFO]")!=0) &&(($token cmp "[INFO]  Begin Timings")!=0)){
...
if(($token11 cmp "[INFO]  Avg time per fwd+bwd")==0){
	...
	#	  if (($token11 cmp "[INFO]  Model")==0){	
		...
		#remove model and plus one stage !!!!!!!!!!!!!   remove    count ++!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
#} 
        ...
        $row2++; 
		if(($count22==0)||($count22!=$stageCount2)){
		$count22++; }
		}
        open (TABFILE, 'alexnet_2.log') or die "alexnet_2.log: $!";
        $row2=0;
...
#	  $worksheet1->write($row1122-51,10, $test3);
#	 }if(	$flag41122==0){
